...
const addQuoteItemBodySchema = z.object({
  ...
  materialCalcMode: z.enum(['SIMPLE_CUT', 'NEST_UNITS']).optional().default('NEST_UNITS'),
  // Nest CHAPA
  sheetCount: z.number().int().min(1).optional().default(1),
  hasPartialLastSheet: z.boolean().optional().default(false),
  partialSheetWidth: z.number().positive().optional().nullable(),
  partialSheetHeight: z.number().positive().optional().nullable(),
  chargeFullLastSheet: z.boolean().optional().default(false),
  // Nest PERFIL
  profileBarCount: z.number().int().min(1).optional().default(1),
  hasPartialLastProfileBar: z.boolean().optional().default(false),
  partialProfileLength: z.number().positive().optional().nullable(),
  chargeFullLastProfileBar: z.boolean().optional().default(false),
  scrapNotes: z.string().optional().nullable(),
  // Geral
  isMaterialProvidedByClient: z.boolean().optional().default(false),
  usagePercentage: z.number().min(0).max(100).optional().nullable(),
  cuttingGasId: z.uuid(),
  cuttingTimeMinutes: z.number().min(0),
  chargeMinimumCutting: z.boolean().optional().default(false),
...
export class AddQuoteItemController {
  ...
  async handle(
    ...
    const result = await this.addQuoteItem.execute({
      ...
      materialCalcMode: body.materialCalcMode,
      sheetCount: body.sheetCount,
      hasPartialLastSheet: body.hasPartialLastSheet,
      partialSheetWidth: body.partialSheetWidth ?? null,
      partialSheetHeight: body.partialSheetHeight ?? null,
      chargeFullLastSheet: body.chargeFullLastSheet,
      profileBarCount: body.profileBarCount,
      hasPartialLastProfileBar: body.hasPartialLastProfileBar,
      partialProfileLength: body.partialProfileLength ?? null,
      chargeFullLastProfileBar: body.chargeFullLastProfileBar,
      scrapNotes: body.scrapNotes ?? null,
      isMaterialProvidedByClient: body.isMaterialProvidedByClient,
      usagePercentage: body.usagePercentage ?? null,
      cuttingGasId: body.cuttingGasId,
      cuttingTimeMinutes: body.cuttingTimeMinutes,
      chargeMinimumCutting: body.chargeMinimumCutting,
